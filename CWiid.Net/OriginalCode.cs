...
namespace monowii
{
    public static class Bluetooth
    {
        ...
        public struct bdaddr_t
        {
            ...
            public bdaddr_t(string addressString)
            {
                ...
                {
                    this.b0 = byte.Parse(addressSegments[0]);
                    this.b1 = byte.Parse(addressSegments[1]);
                    this.b2 = byte.Parse(addressSegments[2]);
                    this.b3 = byte.Parse(addressSegments[3]);
                    this.b4 = byte.Parse(addressSegments[4]);
                    this.b5 = byte.Parse(addressSegments[5]);
                }
            }
    ...
    public static class cwiid
    {
        ...
        private struct cwiid_bdinfo
        {
            private Bluetooth.bdaddr_t bdaddr;

            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
            private byte[] btclass;

            [MarshalAs(UnmanagedType.ByValArray, SizeConst = BT_NAME_LEN)]
            private char[] name;
        }
        ...
        public struct cwiid_mesg
        {
            ...
            [FieldOffset(0)]
            public cwiid_status_mesg status_mesg;

            [FieldOffset(0)]
            public cwiid_btn_mesg btn_mesg;

            [FieldOffset(0)]
            public cwiid_acc_mesg acc_mesg;

            [FieldOffset(0)]
            public cwiid_ir_mesg ir_mesg;

            [FieldOffset(0)]
            public cwiid_nunchuk_mesg nunchuk_mesg;

            [FieldOffset(0)]
            public cwiid_classic_mesg classic_mesg;

            [FieldOffset(0)]
            public cwiid_error_mesg error_mesg;
        }
        ...
        {
            [FieldOffset(0)]
            public nunchuk_state nunchuk;

            [FieldOffset(0)]
            public classic_state classic;
        }
        ...
        #endregion Error reporting (library wide)

        #region Connection

        /// <summary>
        /// Establish a connection with a wiimote. Pass the address of a bdaddr set to *BDADDR_ANY to
        /// connect to any (one) available wiimote. In this case, the bdaddr will be filled with the
        /// address of the connected wiimote. Due to the bizarre way in which BlueZ implements
        /// BDADDR_ANY, it is recommended that BDADDR_ANY not be passed directly to wiimote_connect,
        /// as this may change the value of BDADDR_ANY.
        /// </summary>
        /// <param name="bdaddr">A <see cref="Bluetooth.bdaddr_t"/></param>
        /// <param name="flags">A <see cref="CWIID_FLAGS"/></param>
        ...
        /// <summary>
        /// Accessor function for the wiimote id, which is guaranteed to be unique among all wiimotes
        /// in a single process.Accessor function for the wiimote id, which is guaranteed to be
        /// unique among all wiimotes in a single process.
        /// </summary>
        [DllImport("libcwiid.so")]
        ...
        /// <summary>
        /// Sets the message callback function, which is called when a set of messages are received
        /// from the wiimote (each message in a set was generated from the same packet and therefore
        /// arrived simultaneously). Note that option flag CWIID_FLAG_MESG_IFC must be enabled for
        /// the callback to be called. Passing NULL for the callback cancels the current callback, if
        /// there is one.
        /// </summary>
        [DllImport("libcwiid.so")]
        ...
        ///
        /// CWIID_CMD_STATUS Request a status message (delivered to the message callback) (flags
        /// ignored) CWIID_CMD_LED Set the LED state. The following flags may be bitwise ORed:
        ///
        /// * CWIID_LED1_ON
        /// * CWIID_LED2_ON
        /// * CWIID_LED3_ON
        ...
        /// CWIID_CMD_RPT_MODE Set the reporting mode of the wiimote, which determines what wiimote
        /// peripherals are enabled, and what data is received by the host. The following flags may
        /// be bitwise ORed (Note that it may not be assumed that each flag is a single bit -
        /// specifically, CWIID_RPT_EXT = CWIID_RPT_NUNCHUK | CWIID_RPT_CLASSIC):
        ///
        /// * CWIID_RPT_STATUS
        /// * CWIID_RPT_BTN
        /// * CWIID_RPT_ACC
        ...
        // int cwiid_get_bdinfo_array(int dev_id, unsigned int timeout, int max_bdinfo, struct
